...
name: plan
description: >-
  Create a plan for any task, scaled to its complexity — classify task type
  (chore/feature/refactor/fix/enhancement/content) and complexity, then write a plan.md
  under planning/tasks/plan-<slug>/, omitting heavy sections for simple work. Use as the
  general-purpose planner when the work doesn't clearly fit the chore or feature skill.
---
...
A description of the task to plan, taken from the user's request.

## Instructions

1. If no description is provided, stop and ask the user to describe the task.
...
Next (optional — decompose into atomic sub-steps):
  breakdown skill → planning/tasks/plan-{name}/plan.md

Next (skip breakdown — implement directly):
  implement skill → planning/tasks/plan-{name}/plan.md
```
