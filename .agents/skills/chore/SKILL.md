...
name: chore
description: >-
  Plan a maintenance or housekeeping task for the project (e.g. remove a secret,
  fix typos, prune dead links, update stale handles) — research the codebase and write
  a tasks.md spec under planning/tasks/chore-<slug>/ with a validation block. Use when
  the user describes housekeeping/cleanup work and wants it planned (not a new feature).
---
...
A description of the chore to plan, taken from the user's request.

## Instructions

1. If no description is provided, stop and ask the user to describe the chore.
...
Next (optional — decompose into atomic sub-steps):
  breakdown skill → planning/tasks/chore-{name}/tasks.md

Next (skip breakdown — implement directly):
  implement skill → planning/tasks/chore-{name}/tasks.md
```
