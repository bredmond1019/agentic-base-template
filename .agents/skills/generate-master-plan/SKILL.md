...
0. **If `planning/state.json` does not exist, create it first** — nothing else does. `base-template`'s
   scaffold ships no `state.json`, and there is no `mev init` command that writes one; if this step
   skips creating it, every later `mev emit-state --write` (this command's own "State Refresh" step,
   and every future `/log-work`) silently has nothing to derive from — the block graph never surfaces
   and the project's cross-repo visibility stays dark until someone notices and hand-authors the file
   from scratch (confirmed happening more than once, most recently `client/jardins-fitness`). Write the
   **minimal leaf template** from `docs/state/state-schema.md` § "Leaf project template", scripted —
   never free-hand JSON authoring, which is exactly the class of mistake this needs to not be:
   ```bash
   python3 - <<'PYEOF'
   import json
   ...
   PYEOF
   ```
   Substitute the real slug and today's date for the placeholders before running. Skip this step
   entirely if `planning/state.json` already exists — never overwrite one that's already populated.
...
4. **If `master-plan.md` has no `<!-- BEGIN generated:wave-table -->` sentinel pair, add one** —
   `mev emit-state --write` (the next step) silently skips a file missing it (`W_EMIT_NO_SENTINEL`)
   rather than inventing one into arbitrary prose, so a plan authored without this block never gets
   its live status/dependency view, ever, and nothing says so out loud. Append near the end of the
   file (after the last phase section):
   ```markdown
   ---

   ## Block Status (generated)

   > Regenerated by `mev emit-state --write` from [`state.json`](state.json) — **do not hand-edit
   > inside the sentinels.** This is the live status/dependency view, derived from the block graph.
   > Everything else in this file — the block definitions above — stays authored.

   <!-- BEGIN generated:wave-table -->
   <!-- END generated:wave-table -->
   ```
   Leave the block body empty — the next step fills it. Skip this step if the sentinel pair already
   exists anywhere in the file.

### State Refresh

Run `mev emit-state --write` to update the brain's focus derivation and state based on the new planning files. This is what actually fills the wave-table sentinel (and, if this repo has a
`docs/projects/<slug>.md` cache with its own `<!-- BEGIN generated:project-cache -->` sentinel, that
too) from what steps 0–4 just wrote — an empty sentinel pair with no populated content after this
runs means the sentinel or `state.json` still isn't right, not that there's nothing to show.
...
state.json: <created | already existed>, <N> blocks registered
wave-table sentinel: <added | already present> — populated by mev emit-state --write
