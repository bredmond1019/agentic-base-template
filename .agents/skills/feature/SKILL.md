...
name: feature
description: >-
  Create a comprehensive plan to implement a new feature on the project — design,
  user story, phased implementation, testing strategy — written as a tasks.md spec under
  planning/tasks/feature-<slug>/. Use when the user wants to add a new capability (e.g.
  RSS feed, search, newsletter signup) and needs a full feature plan before building.
---
...
A description of the feature to plan, taken from the user's request.

## Instructions

1. If no description is provided, stop and ask the user to describe the feature.
...
Next (optional — decompose into atomic sub-steps):
  breakdown skill → planning/tasks/feature-{name}/tasks.md

Next (skip breakdown — implement directly):
  implement skill → planning/tasks/feature-{name}/tasks.md
```
