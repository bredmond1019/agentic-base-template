...
name: begin-session
description: >
  Open a named operator session, work it with the operator, and close it by producing the one artifact that ends it. A session is the unit for work that needs the human's judgement — defined entry, defined exit, dies when done. It is not a block with a person in it.
---
