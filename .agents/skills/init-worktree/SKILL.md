...
name: init-worktree
description: >-
  Create an isolated git worktree for a SDLC spec or task under trees/<slug>[-taskN]/ —
  a --no-checkout worktree with cone-mode sparse checkout of the source/content/test/planning
  trees, copied .env files, and a seeded empty commit. Use when the user wants to run a pipeline
  in isolation on its own branch (e.g. "init a worktree for 1.3-projects-add-current task 3").
---
...
Spec slug with optional task number, taken from the user's request. Examples:
...
`2.2-learn-paths-accuracy-refresh`). This matches the worktree naming the SDLC task pipeline uses,
so the **clean-worktree** skill can find and merge whatever this skill (or that pipeline) created.
...
2. **Parse arguments:** split the input on whitespace. First token is `specSlug`. If a second token exists and is a number, it is `taskNum`; otherwise no task number.
...
4. **Verify CWD is repo root:**
   ...
   ```
   If the output does not match the current directory, stop with: "Run this from the repo root, not from inside a subdirectory."
...
5. **Check for name collision:**
   ...
   ```
   - If `trees/<worktreeName>` appears in worktree list → stop: "Worktree '<worktreeName>' already exists. Run the **clean-worktree** skill first."
...
12. **Report success** and print next-step instructions:
    ...
    To run the SDLC pipeline in isolation:
      1. Open a new session with working directory set to:
           <absolute-path-to-repo>/trees/<worktreeName>
      2. Run the pipeline for <specSlug>[ <taskNum>] there.
    ...
    When the pipeline is done, return to the main repo session and run the
    clean-worktree skill with the same args.
    ```
...
- **`node_modules/` is not part of the checkout and is not shared between worktrees.** Run `npm ci` inside the worktree before `npm test` / `npm run build`.
...
- When the pipeline finishes, run the **clean-worktree** skill from the main repo session to merge the branch and clean up.
