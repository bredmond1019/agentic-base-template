...
name: test
description: >-
  Run the validation suite (lint, tsc, validate:content, jest discovery, full
  jest, next build — plus bilingual-parity and link checks for content specs) and return
  results as a JSON array, writing a pipeline test report when a spec path is given. Use
  when the user says "test", "run the validation suite", "run the checks", or after
  implementing a spec.
---
...
Optional path to the task spec and optional task number, taken from the user's request. Same
format as the **implement** skill. Examples:
...
- **Step 0 — Parse the input:** If provided, split on the last space. Trailing number = task N; remainder = spec path. Derive the report file path from the spec's parent directory:
...
- Orient first (the `prime` skill's read-list): read `README.md`, `CLAUDE.md`, `planning/CONTEXT.md`, `planning/STATUS.md` to load standing rules and architecture before executing any tests.
...
If a spec path was provided, after returning the JSON array to chat, write a report file to the
...
invoke the review-task skill on <spec file path> [N]
```
...
```
Next: invoke the review-task skill on planning/tasks/1.1-site-credibility-fixes/tasks.md [N]
```
