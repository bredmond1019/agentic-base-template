...
`sdlc/state.json` are **committed**, exactly as the engines commit theirs — in a vaulted repo through the REAL vault path (`git -C <vault>/planning ...`), never through the `planning/` symlink face, which aborts the whole `git add` with "beyond a symbolic link" (D46). Let M = total number of checks run, including the emoji gate,
and n = number passed.
