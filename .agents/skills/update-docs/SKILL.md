...
name: update-docs
description: >-
  Patch docs/*.md to reflect recent source changes, scoped by a git-diff range
  (ad-hoc, ungated counterpart to the document skill). Surgically updates only doc sections
  describing changed code; flags architecture-level changes (shared libraries, build/config
  files, routing/middleware) as NEEDS_REVIEW. Use for doc maintenance outside
  the SDLC pipeline — "update the docs for my recent changes". Never touches planning/, DEVLOG,
  STATUS, CLAUDE.md.
---
...
diff range rather than a pipeline report. This is the **ad-hoc** counterpart to the **document**
skill (which gates on a review verdict and reads the implement report). Use **update-docs** for
doc maintenance outside the SDLC pipeline; use **document** inside it.

## Inputs

Optional git ref or range to diff against, taken from the user's request (e.g. `HEAD~3`,
`main..HEAD`, a commit hash).

## Instructions

1. Determine the diff range:
   - If a git ref/range is provided, use it (e.g. `HEAD~3`, `main..HEAD`, a commit hash).
   - Otherwise default to `HEAD~1` (everything in the last commit) plus any unstaged changes.
   - Run `git diff --stat <range>` to get the list of changed source files.
