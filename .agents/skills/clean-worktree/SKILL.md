...
- **`/orchestrate` does NOT merge `--worktree` runs for you.** Its Integrate step routes any `--worktree` run to `/clean-worktree <spec-slug>` (`orchestrate.md`, Integrate), so run this command for those blocks too — the engines commit inside the worktree but never merge it back, which is why three measured `/orchestrate` runs each needed an explicit `/clean-worktree`. In-place `sdlc-task` runs have nothing to merge; `sdlc-flow` terminates in a PR.
