...
name: implement
description: >-
  Execute a plan/spec file against the codebase — run all steps (or a single
  numbered task), honoring code conventions, bilingual parity, the public-narrative rule,
  and no-fabricated-metrics; then run the validation gate and write an implement report.
  Use when the user says "implement", "build this", or "execute" a planning/tasks/<name>
  spec, optionally for one task (e.g. "implement task 3").
---
...
Path to the plan file to implement, with an optional task number suffix, taken from the
...
1. If no input is provided, stop and ask the user for the plan file path.
2. Parse the input: split on the last space. If the trailing token is a number, treat it as the **task number** to run in isolation; the remainder is the plan file path. If no number is present, run all tasks.
3. Orient first (the `prime` skill's read-list): read `README.md`, `CLAUDE.md`, `planning/CONTEXT.md`, `planning/STATUS.md`, and run `git ls-files` to load standing rules and architecture before touching any code.
...
```
Next: invoke the test skill on planning/tasks/1.1-site-credibility-fixes/tasks.md [N]
```
