...
name: process-tasks
description: >-
  Analyze the block sequence in planning/STATUS.md and report which spec is
  eligible to start next, applying sequential-prerequisite rules (a Not-started block
  is ready only if every block above it is Done or Skipped). Use when the user asks
  "what can I start", "what's eligible", "what's unblocked", or "process tasks".
