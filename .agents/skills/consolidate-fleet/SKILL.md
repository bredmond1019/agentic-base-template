---
name: consolidate-fleet
description: >
  Reads every unconsolidated lane-log line and run record across a set of roadmaps, plus the commander's retros and carryover triage, and produces one mechanism-level pattern analysis for improving the orchestration system itself. Advances a per-roadmap watermark. Writes no state.json.
---
