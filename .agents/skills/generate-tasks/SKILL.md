...
name: generate-tasks
description: >-
  Generate a task spec (tasks.md) for a master-plan phase/block in the project,
  reading the relevant section of planning/MASTER_PLAN.md and writing a full spec with
  bilingual-parity tasks, acceptance criteria, and a standard validation block. Use
  when the user says "generate tasks", "create the spec for <P.N-slug>", or wants to
  materialize a master-plan block (e.g. 1.3-projects-add-current) into an executable spec.
---
...
The spec's `planning/tasks/` directory name (its phase-dotted slug), e.g.
`1.3-projects-add-current` or `2.1-learn-paths-structural-fixes`, taken from the user's
request. New master-plan specs follow the `P.N-slug` convention (see `planning/README.md`
→ *Task directory naming convention*); ad-hoc work uses the **chore**, **feature**, or
**plan** skills instead. Required — if not given, stop and say:
"Usage: generate-tasks <P.N-slug>  (e.g. 1.3-projects-add-current)". A trailing `--force`
overwrites an existing spec.

## Instructions

1. Orient first (the `prime` skill's read-list): read `README.md`, `CLAUDE.md`,
   `planning/CONTEXT.md`, `planning/STATUS.md` for standing rules and architecture.

2. Parse the input to extract the phase number and block/project identifier
...
3. Check whether a spec already exists at `planning/tasks/phaseN-blockX/tasks.md` (using the
   ...
   - If it exists, read it and report: "Spec already exists at <path>. Overwrite? (re-run with
     `--force` appended to overwrite, or run the **breakdown** skill on `<path>` to decompose it instead.)"
   - If the input contains `--force`, proceed and overwrite.
...
7. **Commit the spec.** Leave the working tree clean so a downstream pipeline run never trips
...
8. Report the path written and suggest the next step:
   "Spec written and committed to planning/tasks/phaseN-blockX/tasks.md. Invoke the **breakdown** skill on planning/tasks/phaseN-blockX/tasks.md to decompose into atomic sub-steps."
...
Next (optional — decompose into atomic sub-steps):
  breakdown skill → planning/tasks/1.1-site-credibility-fixes/tasks.md

Next (skip breakdown — implement directly):
  implement skill → planning/tasks/1.1-site-credibility-fixes/tasks.md
```
