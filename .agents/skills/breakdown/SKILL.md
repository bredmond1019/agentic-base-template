...
name: breakdown
description: >-
  Decompose a task spec into agent-executable sub-steps, reading the actual
  source files each step touches so every sub-step names exact paths, function names,
  and concrete changes (precise enough to execute without interpretation). Writes
  breakdown.md beside the spec. Use when the user says "break down", "decompose", or
  "make this spec executable" for a planning/tasks/<name>/tasks.md.
---
...
Path to the task spec to break down (e.g. `planning/tasks/1.1-site-credibility-fixes/tasks.md`),
taken from the user's request. If omitted, default to the current block's spec identified via
`planning/STATUS.md`. If no spec exists for the current block, say so and suggest running the
...
- The spec file (or the current block's spec)
- `planning/STATUS.md` (only if no spec path was provided)
- `CLAUDE.md`
