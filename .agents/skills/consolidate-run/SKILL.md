---
name: consolidate-run
description: >
  Discovers per-(repo x roadmap) orchestration-run records for one roadmap, cross-checks them against lane-log.jsonl, selects on D57's two-axis origin_roadmap rule, and proposes carryover[] entries with finding_id for mev's cross-repo correlation. Writes no state.json.
---
