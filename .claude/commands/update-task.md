...
Examples:
  ...
  `/update-task 0 Still investigating the pt-BR parity gap`                       ← auto-detect spec, note only
  `/update-task 1.1-site-credibility-fixes 2 Fixed the retired model id in frontmatter`  ← explicit spec, mark step 2, append note
  `/update-task 1.1-site-credibility-fixes 0 Investigating the pt-BR parity gap`      ← explicit spec, note only
...
1. **Resolve the target spec.**
   - If the first token of `$ARGUMENTS` matches a spec identifier pattern (e.g. `1.1-site-credibility-fixes`,
