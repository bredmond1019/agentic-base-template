...
This command is **`brain.toml`-driven and depth-agnostic**. It works unchanged at the brain root, inside a
...
> a future Bastion harness. This command is the deliberate, operator-invoked version.
...
`planning/brain-rag-improvements "RAG corpus-cleanup blocks A–G; H paused for local-embed swap"`.
...
2. **Locate the owning `planning/`.** The archive target belongs to exactly one `planning/` — the nearest
   ...
   **promotion destinations**). If `ARCHIVE_DIR` doesn't exist, create it with an `index.md`
   (`type: Index`, `status: archived`, the "Folder · What it was · Status" table header — model it on
   `planning/archive/index.md`).
...
- Governed by [D35](file://~/agentic-portfolio)
  (the loop) and [D30](file://~/agentic-portfolio)
  (the file pack it promotes into).
...
- `ARCHIVE_DIR/index.md` and the parent `planning/index.md` (D17 propagation)
- [D35](file://~/agentic-portfolio) — the routing table + provenance format
