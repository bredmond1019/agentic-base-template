---
type: Command
title: dispose-run — file a consolidation's mechanisms into the graph
description: Reads a pattern analysis and its disposal.json and files each row as a block, a carryover entry, an operator edge, or explicitly nothing. Refuses rows the evidence does not support and reports them. Files rows and stops; it never authors a roadmap.
doc_id: dispose-run
layer: [factory]
project: base-template
status: active
keywords: [disposal, consolidation, mechanisms, blocks, carryover, filing]
related: [consolidate-fleet, block-registration, D43-cross-domain-priority-graph]
---
