---
name: run-the-gates
description: How to run this fleet's validation gates and get an answer you can trust — why validate-brain's flags must be one per invocation, why a piped command's exit code is the pipe's and not the command's, why a red gate is often another session's file rather than your change, and which checks deliberately do not gate. Use BEFORE running validate-brain, harness.json checks, or a push gate, and whenever a gate result looks wrong, unrelated to your change, or suspiciously green.
allowed-tools: Bash(bastion:*) Bash(mev:*) Bash(git:*) Bash(bash:*) Bash(grep:*) Bash(ls:*)
---
