...
description: Find out what else breaks before you change a doc, a doc_id, a Rust symbol, or a crate this fleet path-depends on — which of the four instruments answers which question, and why the most obvious one (bastion brain) returns a clean-looking WRONG answer on almost every document in this corpus. Use BEFORE renaming or deleting a markdown file or doc_id, before changing a public Rust signature in mev/okf-core/engine-rs/claude-code-rs/bella, before removing something that looks unused, and whenever you are about to say "nothing references this".
allowed-tools: Bash(mev:*) Bash(bastion:*) Bash(python3:*) Bash(jq:*) Bash(rg:*) Bash(grep:*) Bash(cargo:*)
---
