---
name: create-node
description: Guardrail before writing any new Node (atom) in engine-rs or feli — check whether it already exists, whether its INPUT AND OUTPUT SHAPE is a declared contract rather than an ad-hoc dict, whether it should compose an existing injectable seam instead of a hardcoded value, and whether it's actually an LLM-calling node (route to create-llm-node instead/also). Use BEFORE writing a new struct implementing `crate::node::Node` or a new feli `Node` subclass, before declaring a node's output shape, and before adding a new field to an existing node that hardcodes a string/name/path a future caller might want to vary.
