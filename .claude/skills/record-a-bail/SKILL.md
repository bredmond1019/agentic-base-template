---
name: record-a-bail
description: How to classify a bail at the moment it happens so the record on disk can later be counted — the artifact that broke versus the check that noticed and why the artifact is the cluster key, self versus foreign ownership against the task's declared files[], when two differently-worded bails are the same defect and when they are not, and the no-artifact case. Use whenever a block/task bails (engine-driven or hand-driven via /orchestrate) and a `bails[]` entry must be written — before writing check_id, failing_artifact, ownership, bail_class or reason.
allowed-tools: Bash(mev:*) Bash(git:*) Bash(grep:*) Bash(rg:*) Bash(ls:*) Bash(cat:*)
---
