---
name: brain-graph
description: Drive `bastion brain` (the OKF wikilink structural graph) and `bastion code` (tree-sitter Rust symbol graph) — every flag, the exactly-one-of query rule for each, the greppable output grammar, and --json vs --json-logs. Use BEFORE running either verb by hand, when writing a recipe or script that shells them, and when deciding which query flag (--dependents/--blast-radius/--lineage, or --def/--refs/--dependents) answers the question at hand. For WHICH INSTRUMENT (bastion brain vs mev's related: graph) to use in the first place, see check-blast-radius — that trap is not repeated here.
