...
// true.
function buildBailPayload(taskNum, t, attempt, majorFallback, exhaustionFallback = null, gateResultsFreshThisAttempt = false) {
...
`, withModel({ label: `${isFix ? 'fix' : 'implement'}-${taskNum}-${attempt}`, schema: STAGE_SCHEMA, phase: 'Tasks' }, isFix ? fixModel : MODEL.implement))
    ...
    if (!stageResult) {
      ...
      const nullBailPayload = buildBailPayload(taskNum, t, attempt, 'agent returned null', null, false)
    ...
    if (!t.workAssertionPassed) {
      ...
      const waBailPayload = buildBailPayload(taskNum, t, attempt, `Task ${taskNum}: work assertion not confirmed`, null, false)
    ...
    if (vaultRelPaths.length) {
      ...
      if (!vaultVerify.allCommitted) {
        ...
        const vaultBailPayload = buildBailPayload(taskNum, t, attempt, `Task ${taskNum}: vault commit incomplete — ${uncommitted.join(', ')}`, null, false)
    ...
    if (!scanResult.instrumentOk) {
      ...
      const scanBailPayload = buildBailPayload(taskNum, t, attempt, `Task ${taskNum}: removed literal survives outside files[] — ${hitList}`, null, false)
    ...
    const exhaustionFallback = attempt === MAX_TASK_ATTEMPTS
      ...
      : null
    const testBailPayload = buildBailPayload(taskNum, t, attempt, majorFallback, exhaustionFallback, true)
