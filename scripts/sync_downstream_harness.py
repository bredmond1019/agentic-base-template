...
CLAUDE_SKILL_SLUGS: list[str] = [
    # DELIBERATELY EMPTY since BT.chore.skills-go-global (2026-09-03).
    #
    # The 17 fleet-mechanism skills now install ONCE into ~/.claude/skills/ instead of being
    # copied into every repo. Measured before the move: 323 byte-identical copies across 19
    # repos, ZERO divergence, and zero name collisions with any repo-local skill - so nothing
    # was overriding a fleet skill and nothing broke on removal.
    #
    # Why per-repo copies are not merely redundant but INERT once a global install exists:
    # skills, unlike commands, do not offer a picker. Tested 2026-09-03 by planting a
    # same-named skill in learn-ai/.claude/skills/ with a different body and asking the model
    # to invoke it - the GLOBAL body loaded (SKILL_BODY_FROM_GLOBAL). A repo-local copy of a
    # fleet skill name is unreachable, so keeping 323 of them bought staleness and nothing else.
    #
    # CONSEQUENCE, and it is a real cost: a repo CANNOT override or pin a fleet skill. If a
    # repo needs different behaviour it must use a DISTINCT skill name, not shadow a fleet one.
    # (Commands are different: both copies register and the picker shows (user)/(project),
    # with the user entry listed first.)
    #
    # The trade: this moves .claude/skills off sync_downstream_harness.py - the only propagation
    # path in the fleet with an integrity model - onto a per-machine surface with no CI
    # equivalent. That is paid for by the non-gating `global-skills-fresh` check in
    # planning/harness.json. Non-gating is not laziness: a CI checkout has no global install and
    # a fresh clone would fail instantly, the same reason toolchain-freshness does not gate.
    #
    # To reinstate per-repo copies, restore this list from git history and re-run with --apply.
]
