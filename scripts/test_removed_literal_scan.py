...
through node, executed in a throwaway git repo):
  ...
  3. CROSS-HUNK ADDITION (fix for the carryover below) -- a literal removed from one file/hunk that
     is newly referenced on a '+' line in an UNRELATED file/hunk of the same commit is still
     reported: the removed/added subtraction is scoped per hunk, not across the whole diff.

Formerly a known gap, now fixed (carryover removed-literal-scan-same-commit-subtraction-is-
untested): a literal removed from one line but newly referenced on a '+' line elsewhere in the same
commit was subtracted too, even across different files/hunks. Case 3 above pins the fix.
"""
import json
import os
import subprocess
...
SUBTRACTION = 'literals |= extract(hunk_removed) - extract(hunk_added)'
NO_SUBTRACTION = 'literals |= extract(hunk_removed)'
...
def make_multi_file_repo(files_before: dict, files_after: dict, test_content: str) -> tuple[str, str]:
    d = tempfile.mkdtemp(prefix='rls-')
    git(d, 'init', '-q')
    git(d, 'config', 'user.email', 't@t')
    git(d, 'config', 'user.name', 't')
    ...
    Path(d, 'tests').mkdir()
    Path(d, 'tests', 'test_src.py').write_text(test_content)
    Path(d, 'tasks.json').write_text(json.dumps([{'task_id': 1, 'files': ['src.py']}]))
    git(d, 'add', '.')
    git(d, 'commit', '-q', '-m', 'base')
    base = subprocess.run(['git', 'rev-parse', 'HEAD'], cwd=d, capture_output=True, text=True).stdout.strip()
...
class RemovedLiteralScan(unittest.TestCase):
    def test_in_place_edit_does_not_report_surviving_tokens(self):
        ...
        inverted = run_scan(body.replace(SUBTRACTION, NO_SUBTRACTION), d)
    ...
    def test_cross_hunk_addition_does_not_cancel_removal(self):
        # src.py removes GAMMA_CONST; the SAME commit adds GAMMA_CONST in an unrelated file
        # (other.py). Before the per-hunk fix, the whole-diff subtraction cancelled the removal
        # because the literal reappeared anywhere on the diff's '+' side -- the exact gap the
        # carryover named. It must still be reported as removed.
        d, base = make_multi_file_repo(
            {'src.py': 'run("GAMMA_CONST")\n', 'other.py': 'pass\n'},
            {'src.py': 'run()\n', 'other.py': 'x = "GAMMA_CONST"\n'},
            'assert "GAMMA_CONST"\n',
        )
        out = run_scan(render_script(base, 'tasks.json', 1), d)
        self.assertIn('HIT:GAMMA_CONST|tests/test_src.py|1', out, out)
