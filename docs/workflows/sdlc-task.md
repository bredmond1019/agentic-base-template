...
`bails[]` is append-only — one entry per bail (`occurred_at, task_id, check_id, check_id_raw,
failing_artifact, ownership, bail_class, reason, resolution`), never truncated or overwritten;
`bail_reason` stays as a plain mirror of the newest entry's `reason`, null when `bails` is empty.
`resolution` starts `null` and is set to `"resumed-clean"` when a later `--resume` carries an open
entry forward and that task then passes. `check_id` is resolved at write time from the task's own
recorded `gate_results` (the last `'fail'` entry) against `planning/harness.json`'s check names — a
match is stored verbatim, a non-match (or no `gate_results` entry at all, e.g. a non-test bail)
stores `check_id: null` with the raw candidate preserved in `check_id_raw`, never silently dropped.
See
...
skip in the implement step; the terminal bookkeep write also references `workAssertionPassed` so
that a block/task can never close without the per-task gate having already run
