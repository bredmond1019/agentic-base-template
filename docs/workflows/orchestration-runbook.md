...
| A fix you made mid-run doesn't seem to take effect — or a change you reverted seems to | The run executed a cached engine/command copy, not the working tree | `base-template/AGENTS.md` standing rule 10 — hash the engine copy after every launch, or launch the engine from its file; for command files a restart is the only refresh |
